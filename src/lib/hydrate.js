// src/lib/hydrate.ts
import { getJSON, imgSrc, fmtMoney, percent, linkIcon, socialIconsHtml, escapeHtml, escapeAttr, dateRange, formatDate } from './api';
...
// --- Individual sections ---
async function hydrateCauses(root) {
    const list = await getJSON('/causes');
...
async function hydrateVolunteers(root) {
    const list = await getJSON('/volunteers');
...
async function hydrateEvents(root) {
...
async function hydrateBlogs(root) {
...
async function hydrateTestimonials(root) {
    const list = await getJSON('/testimonials');
...
async function hydrateContact(root) {
    const c = await getJSON('/contact');
    if (!c)
        return;
    const headerInfo = safeQuery(root, '.header-top .info');
    if (headerInfo)
        headerInfo.innerHTML = `<li class="ms-4"><a href="mailto:${escapeAttr(c.email || '')}">${escapeHtml(c.email || '')}</a></li>`;
    const headerSocial = safeQuery(root, '.header-top .social-icon');
    if (headerSocial)
        headerSocial.innerHTML = socialIconsHtml(c.socialLinks);
    ...
    if (footerSocial)
        footerSocial.innerHTML = socialIconsHtml(c.socialLinks);
}
async function hydrateCounters(root) {
    const stats = await getJSON('/donations/stats');
    let totalRaised = stats?.totalRaised ?? 0;
    let totalGoal = 0;
    const causes = await getJSON('/causes');
    if (causes) {
    ...
    if (goalEl) {
        const h2 = goalEl.querySelector('h2');
        if (h2)
            h2.innerHTML = fmtMoney(totalGoal);
    }
    if (raisedEl) {
        const h2 = raisedEl.querySelector('h2');
        if (h2)
            h2.innerHTML = fmtMoney(totalRaised);
    }
}
...
export async function hydrateAll(root) {
    await Promise.all([
        hydrateContact(root),
        hydrateCauses(root),
